...
mod mbc5_test {
    ...
    fn test_mbc5_get_rom_bank_1ff() {
        ...
        mbc.set(0xa042, 0x42).unwrap();

        let data = mbc.get(0xa042);
        assert_eq!(data.unwrap(), 0x42);

        mbc.set(0x01f5, 0x00).unwrap();
        assert!(!mbc.ram_lock);
    }
