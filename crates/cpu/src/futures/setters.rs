...
pub async fn data(cpu: Cpu, area: Bits16) -> Result<u8, Error> {
    let data: u16 = cpu.borrow().registers.get(area);
...
pub async fn reg_at(cpu: Cpu, dst: Bits16, src: Bits8) -> Result<u8, Error> {
    let data: u8 = cpu.borrow().registers.get(src);
    let address: u16 = cpu.borrow().registers.get(dst);
...
pub async fn hl(cpu: Cpu, area: Bits8) -> Result<u8, Error> {
    let data: u8 = cpu.borrow().registers.get(area);
    let dst: u16 = cpu.borrow().registers.get(Bits16::HL);
    cpu.memory().set(dst, data).await
}

pub async fn io_c(cpu: Cpu) -> Result<u8, Error> {
    let a = cpu.borrow().registers.get(Bits8::A);
